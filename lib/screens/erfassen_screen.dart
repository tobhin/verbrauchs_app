...
class _ErfassenScreenState extends State<ErfassenScreen> {
  ...
  Reading? _lastReading; // Hält den letzten Wert für die Validierung (kann Startwert oder reguläre Ablesung sein)
  ...
  // ANGEPASST: Lädt jetzt den Startwert und den letzten regulären Wert für die Validierung
  Future<void> _onMeterChanged(Meter? newMeter) async {
    ...
    final startwert = await AppDb.instance.fetchStartwertForMeter(newMeter.id!);

    setState(() {
      _selected = newMeter;
      _selectedMeterType = meterType;
      // Der letzte Wert ist entweder die letzte reguläre Ablesung oder der Startwert
      _lastReading = readings.isNotEmpty ? readings.first : startwert;
    });
  }
  ...
  Future<void> _pickImage() async {
    ...
    try {
      ...
      switch (meterTypeName) {
        case 'Strom (HT/NT)': meterTypeLabel = MeterTypeForOcr.strom; break;
        case 'Wasser': meterTypeLabel = MeterTypeForOcr.wasser; break;
        case 'Schmutzwasser': meterTypeLabel = MeterTypeForOcr.schmutzwasser; break;
      ...
      if (best != null) {
        ...
        messenger.showSnackBar(SnackBar(content: Text('OCR: Wert vorgeschlagen ($best)')));
      } else {
        messenger.showSnackBar(const SnackBar(content: Text('OCR: kein plausibler Wert gefunden')));
      }
    } catch (e) {
  ...
  Future<void> _save() async {
    ...
    final newReading = Reading(
      ...
      date: DateTime.now(),
      ht: _isStromZaehler ? _parseNum(_htCtrl.text) : null,
      nt: _isStromZaehler ? _parseNum(_ntCtrl.text) : null,
      value: !_isStromZaehler ? _parseNum(_valueCtrl.text) : null,
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _selected?.id == m.id ? Theme.of(context).colorScheme.primaryContainer : Theme.of(context).colorScheme.secondaryContainer,
              borderRadius: BorderRadius.circular(16),
              border: _selected?.id == m.id ? Border.all(color: Theme.of(context).colorScheme.primary) : null,
            ),
            child: Column(
              children: [
                Icon(
                  ...
                  children: [
                    if (_isStromZaehler) ...[
                      TextFormField(
                        ...
                        // ANGEPASST: Strikte "größer als" Validierung
                        validator: (v) {
                          final val = _parseNum(v);
                          if (val == null) return 'Bitte eine Zahl eingeben';
                          if (_lastReading?.ht != null && val <= _lastReading!.ht!) return 'Wert muss höher sein als der letzte!';
                          return null;
                        },
                      ),
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText: _lastReading?.nt != null ? 'Letzter Wert: ${_lastReading!.nt}' : 'Optional',
                          border: const OutlineInputBorder(),
                        ),
                        // ANGEPASST: Strikte "größer als" Validierung
                        validator: (v) {
                          if (v != null && v.isNotEmpty) {
                            ...
                            if (_lastReading?.nt != null && val <= _lastReading!.nt!) return 'Wert muss höher sein als der letzte!';
                          }
                          return null;
                        },
                      ),
                      ...
                      TextFormField(
                        ...
                        validator: (v) {
                          final val = _parseNum(v);
                          if (val == null) return 'Bitte eine Zahl eingeben';
                          if (_lastReading?.value != null && val <= _lastReading!.value!) return 'Wert muss höher sein als der letzte!';
                          return null;
                        },
                      ),
                    ],
