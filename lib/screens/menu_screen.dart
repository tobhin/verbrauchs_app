...
class _MenuScreenState extends State<MenuScreen> {
  ...
  final Map<String, IconData> _iconMap = {
    ...
    'heat_pump': Icons.heat_pump, 'question_mark': Icons.question_mark,
  };
  ...
  Future<void> _addMeter() async {
    final selectedType = await showDialog<MeterType>(
      context: context,
      builder: (ctx) => const MeterTypeSelectionDialog(),
    );
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('Neuer Zähler: ${selectedType.name}'),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Name des Zählers (z.B. Keller Wasseruhr)')),
  ...
  Future<void> _showSetStartwertDialog(Meter meter) async {
    ...
    await _showStableInputDialog(
      ...
      fields: [
        ...
        if (meterType?.name == 'Strom') ...[
          const SizedBox(height: 16),
          TextField(keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Anfangszählerstand NT (optional)')),
        ],
        const SizedBox(height: 16),
        TextField(
          controller: dateCtrl,
                    ...
                    children: _meters.map((meter) {
                      return CheckboxListTile(
                        ...
                        onChanged: (bool? value) async {
                          ...
                          await AppDb.instance.updateMeterFavoriteStatus(meter.id!, value);
                          await _loadData();
                        },
                      );
                    }).toList(),
...
class MeterTypeSelectionDialog extends StatefulWidget {
  const MeterTypeSelectionDialog({super.key});

  @override
  State<MeterTypeSelectionDialog> createState() => _MeterTypeSelectionDialogState();
}

class _MeterTypeSelectionDialogState extends State<MeterTypeSelectionDialog> {
  List<MeterType> _defaultTypes = [];
  List<MeterType> _customTypes = [];
  ...
  final Map<String, String> _iconNameDe = {
    'water_drop': 'Wassertropfen', 'bolt': 'Blitz', 'local_fire_department': 'Flamme',
    'thermostat': 'Thermostat', 'solar_power': 'Sonne', 'waves': 'Wellen',
    'heat_pump': 'Wärmepumpe', 'question_mark': 'Fragezeichen',
  };
  ...
  Future<void> _loadTypes() async {
    final defaults = await AppDb.instance.fetchMeterTypes(isDefault: true);
    final customs = await AppDb.instance.fetchMeterTypes(isDefault: false);
    if (mounted) {
      setState(() {
        _defaultTypes = defaults;
        _customTypes = customs;
      });
    }
  }
  ...
  Future<void> _createNewType() async {
    final nameCtrl = TextEditingController();
    String selectedIcon = 'question_mark';

    final newType = await showDialog<MeterType>(
      context: context,
      builder: (ctx) {
        return AlertDialog(
          title: const Text('Neuen Zählertyp erstellen'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Name (z.B. Gartenwasser)')),
              const SizedBox(height: 16),
              DropdownButtonFormField<String>(
                value: selectedIcon,
                items: _iconMap.keys.map((name) => DropdownMenuItem(
                  value: name,
                  child: Row(children: [Icon(_iconMap[name]), const SizedBox(width: 8), Text(_iconNameDe[name] ?? name)]),
                )).toList(),
                onChanged: (v) => selectedIcon = v ?? 'question_mark',
                decoration: const InputDecoration(labelText: 'Icon auswählen'),
              ),
            ],
          ),
          actions: [
            ...
            FilledButton(onPressed: () async {
              ...
              final type = MeterType(name: nameCtrl.text.trim(), iconName: selectedIcon);
              final id = await AppDb.instance.insertMeterType(type);
              Navigator.pop(ctx, MeterType(id: id, name: type.name, iconName: type.iconName));
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Zählertyp auswählen'),
      content: SizedBox(
        width: double.maxFinite,
        child: ListView(
          shrinkWrap: true,
          children: [
            const Text('Standard-Typen', style: TextStyle(fontWeight: FontWeight.bold)),
            const Divider(),
            GridView.builder(
              shrinkWrap: true,
              ...
              itemBuilder: (ctx, i) {
                ...
                return InkWell(
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(_iconMap[type.iconName] ?? Icons.question_mark, size: 32),
            ...
            if (_customTypes.isNotEmpty) ...[
              const SizedBox(height: 16),
              const Text('Meine Typen', style: TextStyle(fontWeight: FontWeight.bold)),
              const Divider(),
              ..._customTypes.map((type) => ListTile(
                leading: Icon(_iconMap[type.iconName] ?? Icons.question_mark),
      ...
      actions: [
        TextButton.icon(
          ...
          label: const Text('Neuen Typ erstellen'),
